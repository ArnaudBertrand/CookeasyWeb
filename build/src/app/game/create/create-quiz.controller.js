(function(){
  ...
  function GameCreateQuizCtrl($state,ceGamesQuizzes){
    ...
    function addAnswer(correct){
      ...
      if(vm.currentQuestion.answers.length < 4){
    ...
    function checkQuestion(){
      ...
      if(q.answers.length > 4) {vm.errors.answers = 'Insert maximum 4 answers';}

      var oneCorrect = false;
      q.answers.forEach(function (a){
        if(a.correct) {return oneCorrect = true;}
      });
      if(!oneCorrect) {vm.errors.answers = 'Have at least one correct answer';}
    }
    ...
    function goTo(id,skipSave){
      ...
      // Stop if there are some errors
      if(Object.keys(vm.errors).length && !skipSave) {return false;}
